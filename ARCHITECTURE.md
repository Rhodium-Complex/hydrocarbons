...
`include_tetrahedral_stereo=True` の場合は、追加で
`stereochemistry.analyze_chiral(...)` を呼び出し、四面体炭素を
`[C@H]`／`[C@@H]` または `[C@]`／`[C@@]` として描画します。二重結合数が
偶数のクムレン鎖は、中央原子に `[C@AL1]`／`[C@AL2]` を付けて描画します。
これは立体配置の列挙であり、CIP による R/S や Ra/Sa の命名ではありません。
ヘリシティは対応範囲外です。

すべての CH 炭素と第四級 sp3 炭素を、潜在的な四面体中心として保持します。
自己同型写像は、局所置換の偶奇を反映して立体配置全体へ作用します。他に選択された
すべての立体配置を保存する局所的な奇置換が存在しない場合に限り、潜在中心を `@` 付きで
描画します。これにより、隣接中心へ立体配置を与えた後に初めて立体中心となる炭素も
扱えます。
