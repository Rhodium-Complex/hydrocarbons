...
def benchmark_generation(max_carbon: int) -> None:
    for carbon_count in range(2, max_carbon + 1):
        ...
        for hydrogen_count in range(0, carbon_count * 2 + 3, 2)[::-1]:
            combinations = list(structure_generator.build_carbon_hydrogen_combination(carbon_count, hydrogen_count))
        ...
        print(
            ...
            f"unique_outputs={total_structures} seconds={elapsed:.4f} peak_bytes={peak}"
        )
...
def benchmark_unique_mols(carbon_count: int, hydrogen_count: int, min_candidates: int, repeats: int) -> None:
    ...
    for repeat_index in range(repeats):
        ...
        print(
            ...
            f"unique_outputs={len(unique)} seconds={elapsed:.4f} peak_bytes={peak}"
        )
...
def main() -> None:
    ...
    print("unique_mols")
    benchmark_unique_mols(args.unique_carbon, args.unique_hydrogen, args.min_candidates, args.unique_repeats)
